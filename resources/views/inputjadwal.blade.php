    ...
    <div class="form-row">
      <div class="form-group col-md-6">
        ...
        <select id="merek" name="merek" class="form-control">
            @foreach ($databus as $countbus)
            <option value="{{ $countbus->id }}">{{ $databus->merek }}</option>
            @endforeach
        </select>
      </div>
      <div class="form-group col-md-6">
        ...
        <select id="supir" name="supir" class="form-control">
          @foreach ($datasupir as $countsupir)
            <option value="{{ $datasupir->id }}">{{ $datasupir->nama_supir }}</option>
            @endforeach
          </select>
      </div>
    </div>
